...
export interface BuyingPowerInputs {
  price: number;
  downPayment: number;
  rent: number;
  tax: number;
...
export const DEFAULT_BUYING_POWER_INPUTS: BuyingPowerInputs = {
  price: 425000,
  downPayment: 106250,
  rent: 3100,
  tax: 400,
...
/**
 * DSCR rental-purchase "buying power" calculator, used on the dedicated
 * /loans/dscr-rental-purchase funnel. All expense inputs are monthly.
 *   Loan amount = price − down payment
 *   P&I = L·r / (1 − (1+r)^−n)
 *   Monthly housing expense = P&I + taxes + insurance + HOA
 *   DSCR = monthly rent ÷ monthly housing expense
 *   LTV = loan amount ÷ price
 */
export function calcBuyingPower(v: BuyingPowerInputs): BuyingPowerResult {
  const loanAmount = Math.max(v.price - v.downPayment, 0);
  const r = v.rate / 1200;
  const n = v.term * 12;
  const pi = r ? (loanAmount * r) / (1 - Math.pow(1 + r, -n)) : loanAmount / n;
  const monthlyExpense = pi + v.tax + v.ins + v.hoa;
  return {
    loanAmount,
    pi,
    monthlyExpense,
    dscr: monthlyExpense ? v.rent / monthlyExpense : 0,
    ltv: v.price ? (loanAmount / v.price) * 100 : 0,
  };
}
